<template lang="pug">
  form.form-horizontal(@submit="submitForm")
    .form-group
      ...
      .col-9
        input.form-input(type="text" v-model="title" placeholder="Type task title...")
    .form-group
      .col-3
        label.form-label Body
      .col-9
        textarea.form-input(v-model="body" rows=8 placeholder="Type your task...")
    .form-group
      .col-3
      .col-9
...
export default {
  ...
  methods: {
    ...
    createTask () {
      // Вызываем действие `createTask` из хранилища, которое
      // отправит запрос на создание новой заметки к нашему API.
      this.$store.dispatch('createTask', { title: this.title, body: this.body })
    }
  }
}
</script>